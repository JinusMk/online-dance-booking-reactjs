                ...
                <Grid container>
                    <Grid item xs={12}>
                        <div className="bottom-blk">
                            ...
                            {checkNumberOfDaysLeft(subscription.endDate) < 0 ? <p><Link className="primaryBtn" to={{pathname: `/subscription/${subscription?.subscription?.slug}/${subscription?.subscription?.category?._id}`, state: { prevPath: `${location.pathname}`}}}>RENEW SUBSCRIPTION</Link></p> : <p><Link to={{pathname: `/user-subscriptions/${subscription._id}/progress`, state: { prevPath: `${location.pathname}`}}} className="primaryBtn">SEE MY PROGRESS</Link></p>}
                        </div>
                    </Grid>
                </Grid>
            </div>
