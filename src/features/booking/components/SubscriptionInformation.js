        ...
        <div className="subscription-information-card">
            ...
            <div className="bottom-blk">
                <ul className="listUnstyled">
                    {
                        subscriptionBenefits[subscription.category?._id].map((point, index) => index <= 1 && <li className="paragraph" key={index}>
                            <img src={`${imageBasePath}booking_success_tick.svg`}/>
                            <span>{point}</span>
                        </li>)
                    }
                    {subscriptionBenefits[subscription.category?._id]?.length > 2 ? <li className="more-point">{`+${subscriptionBenefits[subscription.category?._id]?.length - 2} more benefits`}</li> : null}
                </ul>
            </div>
        </div>
    )
