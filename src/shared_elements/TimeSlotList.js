        ...
        <div className="time-slots-wrapper">
            ...
            <ul className="listInline">
                {timeSlots.map((item, index) => <li key={index}>
                    <Link to={{pathname: `/dance/${danceId}/book`, state: { sectionId: sectionId, selectedDate: date, time: item.time }}} className={`primaryBtn round ${item.status == "Disabled" ? 'disabled': ''}`}>{item.status == "Disabled" ? item.time : `Book ${item.time}`}</Link>
