        ...
        <Hidden only={['lg', 'xl', 'md']}>
            <BottomNavigation
                ...
                >
                    ...
                    <BottomNavigationAction onClick={() => {setValue(2); navigateTo("/profile")}} label="YOU" icon={(props.isLoggedIn && props.userInfo.photoURL) ? <img src={props.userInfo.photoURL} className={`user-avatar ${location.pathname === '/profile' ? 'active': ''}`}/> : location.pathname === '/profile' ?  <img src={`${imageBasePath}profile_icon_active.svg`} /> : <img src={`${imageBasePath}profile_icon.svg`} />}/>
            </BottomNavigation>
        </Hidden>
    )
}
